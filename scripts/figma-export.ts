 ...
 * Tier 1, the most rate-limited tier, so one call for all `WANTED` assets
 * instead of one call each is the difference between finishing and being
 * throttled — which is what happened to the original extraction.
 *
 * The seven bento-grid illustrations are deliberately NOT in `WANTED`: they
 * come from the Figma MCP `get_screenshot` tool instead (see
 * `docs/figma/DESIGN-SPEC.md`'s bento-illustrations note and
 * `src/components/sections/feature-grid/bento-assets.ts`, which records the
 * node-id-to-file mapping), because they need per-card crops rather than a
 * single combined fill. Re-adding a combined bento entry here would
 * duplicate that artwork, not restore it.
 *
...
async function main(): Promise<void> {
  ...
  console.error('figma: fetching image fill URLs (1 Tier 1 request for the whole file)')
  ...
  if (resolved.length !== WANTED.length) {
    ...
    const missing = WANTED.filter((item) => !found.has(item.name)).map((item) => item.name)
  ...
  try {
    for (const item of resolved) {
      manifest.push(await materialize(client, item, previous.get(`${item.name}.webp`), scratchDir))
    }
  } finally {
  ...
  console.error(`figma: manifest written, ${manifest.length} asset(s) in src/assets/figma/`)
}
 ...
 */
function matchRefs(urls: Readonly<Record<string, string | null>>): ResolvedAsset[] {
  const keys = Object.keys(urls)
  const matched: ResolvedAsset[] = []

  for (const wanted of WANTED) {
    const hits = keys.filter((key) => key.startsWith(wanted.ref) || wanted.ref.startsWith(key))
    if (hits.length === 0) {
      console.error(`figma: no fill found for ${wanted.name} (${wanted.ref.slice(0, 12)}…)`)
      continue
    }
    if (hits.length > 1) {
      console.error(`figma: ambiguous ref for ${wanted.name}, skipping (${hits.length} matches)`)
      continue
    }
    const key = hits[0]
...
async function materialize(
  ...
  if (
    ...
    if (existingBytes === previous.bytes) {
      console.error(`figma: ${webpName} already current (${kb(existingBytes)}), skipping`)
      return {
    ...
    console.error(`figma: ${webpName} size differs from the manifest, re-exporting`)
  } else if (existingBytes !== undefined) {
    console.error(`figma: ${webpName} exists but its source or settings changed, re-exporting`)
  }
  ...
  console.error(`figma: ${webpName} ${kb(sourceBytes)} -> ${kb(targetBytes)} (-${saved}%)`)
...
export function sameInputs(
  ...
  return (
    previous.ref === item.ref && previous.maxWidth === maxWidth && previous.quality === item.quality
  )
}

function describe(item: ResolvedAsset, maxWidth: number): Omit<ManifestEntry, 'file' | 'bytes'> {
...
async function readManifest(): Promise<Map<string, ManifestEntry>> {
  try {
    ...
    if (typeof parsed !== 'object' || parsed === null || !('assets' in parsed)) return new Map()
...
function isManifestEntry(value: unknown): value is ManifestEntry {
  ...
  return typeof candidate.file === 'string' && typeof candidate.ref === 'string'
}
